...
public class C206_CaseStudy {
	...
	public static void main(String[] args) {
		...
		serviceList.add(new Service("Cleaning house", "clean backyard", 100.00));
		...
		while (option != OPTION_EXIT) {
			...
			if (option == OPTION_ADD) {
				...
				if (which == 1) {
					...
					C206_CaseStudy.viewAllServiceProvider(SP_List);
				} else if (which == 5) {
				...
				if (which == 1) {
					...
					String name=Helper.readString("Enter your Company Name to delete: ");
				     C206_CaseStudy.deleteServiceProvider(SP_List,name );
				} else if (which == 5) {
					quote quoteinsert = inputQuote();
	...
	private static void doDeleteRequest(ArrayList<Request> requestList) {
		...
		Helper.line(80, "-");
		System.out.println(
				String.format("%-5s %-20s %-20s %-20s %-20s", "ID", "Username", "Service", "Description", "Status"));
		Helper.line(80, "-");
		for (int n = 0; n < requestList.size(); n++) {
			System.out.println(String.format("%-5s %-20s %-20s %-20s %-20s\n", n + 1, requestList.get(n).getname(),
					requestList.get(n).getRequestService(), requestList.get(n).getRequestDescription(),
					requestList.get(n).getRequestStatus()));
		}
	...
	private static void doDeleteUser(ArrayList<User> registeredUsers) {
		...
		Helper.line(80, "-");
		System.out.println(
				String.format("%-5s%-20s %-25s %-15s %-25s", "ID", "Username", "Email", "Contact Number", "Address"));
		Helper.line(80, "-");
		for (int x = 0; x < registeredUsers.size(); x++) {
			System.out.println(String.format("%-5d%-20s %-25s %-15d %-25s\n", x + 1, registeredUsers.get(x).getname(),
					registeredUsers.get(x).getEmail(), registeredUsers.get(x).getContact(),
					registeredUsers.get(x).getAddress()));
		}
		int removeID = Helper.readInt("Please enter the ID to remove:");
...
//test 
	...
	public static User inputupdateUser(ArrayList<User> userList, int updateID) {
		...
		if (update == 1) {
			String newName = Helper.readString("Please enter your new name:");
			name = newName;
		} else if (update == 2) {
			String newEmail = Helper.readString("Please enter your new email:");
			email = newEmail;
		} else if (update == 3) {
			int newContact = Helper.readInt("Please enter your new contact number:");
			contact = newContact;
		} else if (update == 4) {
			String newPassword = Helper.readString("Please enter your new password:");
			password = newPassword;
		} else if (update == 5) {
			String newAddress = Helper.readString("Please enter your new address:");
			address = newAddress;
		}
	...
	public static String retrieveUser(ArrayList<User> userList) {
		...
		for (int x = 0; x < userList.size(); x++) {
			...
			output += String.format("%-5d%-20s %-25s %-15d %-25s\n", x + 1, username, email, contact_number, address);
		}
		return output;
	}
	...
	public static String userLogin(ArrayList<User> registeredUsers) {
		...
		for (int x = 0; x < size; x++) {
			...
			if (username.equals(getname) && password.equals(registeredUsers.get(x).getPassword())) {
	...
	public static String retrieveAllRequest(ArrayList<Request> requesttList, String username) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (request.getname().equalsIgnoreCase(username)) {
				...
				output += String.format("%-5d %-20s %-20s %-20s %-20s\n", requestId, request.getname(), requestService,
	...
	public static void viewAllRequest(ArrayList<Request> requestList, String username) {
		C206_CaseStudy.setHeader("Request List");
		String output = String.format("%-5s %-20s %-20s %-20s %-20s\n", "ID", "Username", "Service", "Description",
				"Status");
		output += retrieveAllRequest(requestList, username);
		System.out.println(output);
	}
	...
	public static Appointment inputAppointment() {
		...
		Appointment A = new Appointment(username, date, time, additional_details);
		return A;
	...
	public static String retrieveAllServiceProvider(ArrayList<Service_Provider> SP_List) {
		// TODO Auto-generated method stub
		String output = "";
		// write your code here
		for (int i = 0; i < SP_List.size(); i++) {
			...
			if (sp!=null) {
				output += String.format("%-20s %-20s %-20s %-20s\n",
						sp.getCompanyName(), sp.getPassword(),sp.getType(),
						sp.getlocation());
			}}
		...
		public static void viewAllServiceProvider(ArrayList<Service_Provider> SP_List) {
			// TODO Auto-generated method stub
			C206_CaseStudy.setHeader("SERVICE LIST");
			String output = String.format("%-20s %-20s %-20s %-20s\n", "Company Name", "Service", "Location","Type");
			output += retrieveAllServiceProvider(SP_List);
			System.out.println(output);
		}

		public static void deleteServiceProvider(ArrayList<Service_Provider> SP_List, String name) {
		   for (int i=SP_List.size()-1;i>=0;i--) {
			   Service_Provider provider=SP_List.get(i);
			   if(provider.getCompanyName().equalsIgnoreCase(name)) {
				   SP_List.remove(i);
				   System.out.println("Service Provider Deleted :3");
				   return;
			   }
		   }
	...
	public static Service inputService(String type) {
		...
		Service s = new Service(type, description, budget);
		return s;
	}
	...
	public static String retrieveAllService(ArrayList<Service> serviceList) {
		...
		for (Service s : serviceList) {
			output += String.format("%-20s %-20s %-20s\n", s.getType(), s.getDescription(), s.getBudget());
		}
		return output;
	}
